...
public class MyUserDetailsService implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		com.abhi.app.model.Profile profile = profileDao.findProfileByMail(username);
		if(profile==null)
			return null;
		List<GrantedAuthority> grantedAuthorities=new ArrayList<GrantedAuthority>();
		grantedAuthorities.add(new SimpleGrantedAuthority(profile.getRole()));
		return new RequestUserDetails(profile.getEmail(),profile.getPassword(),profile.getId(),grantedAuthorities);
